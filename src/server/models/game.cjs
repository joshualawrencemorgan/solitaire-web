...
const KlondykeMoveSchema = new Schema({
  cards: [{ suit: String, value: String, up: Boolean }],
  src: String,
  dst: String,
});
...
let Game = new Schema({
  ...
  state: [KlondykeGameState],
  undostate: [KlondykeGameState],
  game: {
    type: String,
    required: true,
  ...
  moves: [KlondykeMoveSchema], // Added moves array field
});
